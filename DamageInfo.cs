                ...
                foreach (var entry in playerDamageInfo)
                {
                    ...
                    foreach (var (targetId, targetEntry) in entry.Value)
                    {
                        ...
                        if (attackerController != null && targetController != null)
                        {
                            ...
                            PrintToPlayerChat(attackerController, $"===> {ChatColors.Lime}To: [{damageGiven} / {hitsGiven} hits] From: [{damageTaken} / {hitsTaken} hits] - {ChatColors.Gold}{targetName} - ({targetHP} HP){ChatColors.Default}");
                            PrintToPlayerChat(targetController, $"===> {ChatColors.Lime}To: [{damageTaken} / {hitsTaken} hits] From: [{damageGiven} / {hitsGiven} hits] - {ChatColors.Gold}{attackerName} - ({attackerHP} HP){ChatColors.Default}");
                        }
